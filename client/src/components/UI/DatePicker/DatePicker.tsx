...
import { IAvailableDateTimes, IReservation } from '../../../types/Types';
import {
  startOfToday,
  format,
  parse,
...
export const DatePicker: FC = () => {
  ...
  const [schedule, setSchedule] = useState<IAvailableDateTimes[]>();
  const { id } = useParams();
  const [timeSlots, showTimeSlots] = useState(false);
  ...
  const fetchData = async (): Promise<void> => {
    try {
      const response = await axios.get<IAvailableDateTimes[]>(
      ...
      <div className="max-w-md px-4 mx-auto sm:px-7 md:max-w-4xl md:px-6">
        <div className="md:grid md:grid-cols-2 md:divide-x md:divide-gray-200">
          ...
          <section
            ...
            style={{ visibility: timeSlots ? 'visible' : 'hidden' }}
          >
